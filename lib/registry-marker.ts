...
export function inferSavedCustomMarkerPath({
  ...
  // 'equal' is what generateRegistryMarkerDetectionRules() emits today for
  // both comparison types; 'greaterThanOrEqual' is what it emitted for version
  // comparisons before, and profiles saved back then are exactly the ones this
  // function exists to recover, so both count as our own generated shape.
  const generatedOperators = useVersionComparison
    ? ['equal', 'greaterThanOrEqual']
    : ['equal'];
  if (
    rule.detectionType !== (useVersionComparison ? 'version' : 'string') ||
    !generatedOperators.includes(rule.operator as string)
  ) {
    return null;
  }
...
export function reconcileManagedMarkerDetectionRules({
  ...
  return detectionRules.map((rule) => {
    ...
    return {
      ...
      // Must stay in step with generateRegistryMarkerDetectionRules() in
      // lib/detection-rules.ts: equal for both comparison types, never
      // greaterThanOrEqual. This function rewrites managed marker rules
      // wherever they pass through, so a >= here would quietly undo that
      // rule's own-version-only detection.
      operator: 'equal',
